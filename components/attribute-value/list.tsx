...
import { AttributeValueTable } from "@/components/tables";
import { openPopup } from "@/service/modal.service";
import { useAppSelector } from "@/hooks";
import { RootState } from "@/redux/store";
import { CreateUpdateAttributeValueModal } from "../modals";

interface IAttributeValueProps { }

const AttributeValueList: React.FunctionComponent<IAttributeValueProps> = () => {
    const t = useTranslations("AttributeValuePage");
    const { refresh } = useAppSelector(
        (state: RootState) => state.datatable
    );
    const handleOpenModal = async () => {
        await openPopup('attribute-value', `${t('Add')} ${t('Attribute Value')}`, 'add');
    };
        ...
        <div className="space-y-6">
            <div className="flex items-center flex-wrap justify-between gap-4">
                <Breadcrumbs>
                    ...
                    <BreadcrumbItem>{t("Attribute Value")}</BreadcrumbItem>
                </Breadcrumbs>
            </div>
            <div className="space-y-6">
                <div className="flex items-center flex-wrap justify-between gap-4">
                    ...
                    <div className="flex-none flex items-center justify-end gap-4">
                        ...
                        <CreateUpdateAttributeValueModal trans={t} />
                    </div>
                </div>
            </div>
            <div className="grid grid-cols-1 gap-6">
                <Card>
                    <CardContent className="pt-6">
                        <AttributeValueTable key={String(refresh)} trans={t} />
                    </CardContent>
                </Card>
            </div>
        </div>
