...
import { deleteUom, fetchUomById } from "@/service/uom.service";
...
export function RowActions({ row }: RowActionsProps) {
  ...
  const handleRecordDelete = async (uuid: string) => {
    try {
      const response: any = await deleteUom(uuid);
  ...
  const handleOpenModal = async (uuid: string) => {
    try {
      const response: any = await fetchUomById(uuid);
      if (response?.status === true && response?.statusCode === 200) {
        await openPopup('uom', `${t('Edit')} ${t('UOM')}`, 'edit', response.data);
      }
    } catch (error: any) {
      toast.error(error?.message);
    }
