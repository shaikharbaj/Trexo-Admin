...
import { ConsumerTable } from "@/components/tables";
import { useAppSelector } from "@/hooks";
import { RootState } from "@/redux/store";

interface IConsumerProps {}

const ConsumerList: React.FunctionComponent<IConsumerProps> = () => {
  const { refresh } = useAppSelector((state: RootState) => state.datatable);
  const t = useTranslations("ConsumerPage");
    ...
    <div className="space-y-6">
      <div className="flex items-center flex-wrap justify-between gap-4">
        <Breadcrumbs>
          ...
          <BreadcrumbItem>{t("Consumer")}</BreadcrumbItem>
        </Breadcrumbs>
      </div>
      <div className="space-y-6">
        <div className="flex items-center flex-wrap justify-between gap-4">
      ...
      <div className="grid grid-cols-1 gap-6">
        <Card>
          <CardContent className="pt-6">
            <ConsumerTable key={String(refresh)} trans={t} />
          </CardContent>
        </Card>
      </div>
    </div>
