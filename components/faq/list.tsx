...
import { FaqTable } from "@/components/tables";
import { openPopup } from "@/service/modal.service";
import { useAppSelector } from "@/hooks";
import { RootState } from "@/redux/store";
import CreateUpdateFaqModal from "../modals/faq/create-update-modal";
...
const FaqList: React.FunctionComponent<IFaqProps> = () => {
    ...
    const handleOpenModal = async () => {
        await openPopup('faq', `${t('Add')} ${t('FAQ')}`, 'add');
    };
            ...
            <div className="grid grid-cols-1 gap-6">
                <Card>
                    <CardContent className="pt-6">
                        <FaqTable key={String(refresh)} trans={t} />
                    </CardContent>
                </Card>
            </div>
        </div>
